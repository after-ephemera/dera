...
async fn main() -> Result<()> {
    ...
    let app = get_mysten_service(package_name!(), package_version!())
        ...
        .route(
            ...
            post({
                let shared_state = Arc::clone(&state);
                move |body| hook(body, shared_state)
            }),
        )
...
async fn hello(app_state: Arc<AppState>) -> &'static str {
    app_state.metrics.requests.inc();
    "Hello, World!"
}

/// basic handler that responds with a static string
async fn hook(Json(payload): Json<Value>, app_state: Arc<AppState>) -> &'static str {
